...
def _retry_with_backoff(func: Callable, max_retries: int = None, base_delay: float = None) -> Any:
    ...
    Raises:
        RateLimitError: If all retries are exhausted due to 429 errors.
    """
...
def _parse_json_response(text: str, max_retries: int = 2) -> dict[str, Any]:
    ...
    Raises:
        JSONParseError: If JSON cannot be parsed after all retries.
    """
...
class GroqClient:
    ...
    def _call_groq(
        ...
        Returns:
            The completion response (or stream iterator if stream=True).
        """
        if not self.api_key:
    ...
    def generate_json(
        ...
        Uses retry-with-backoff for rate limits and JSON parsing retry.
        """
        def _do_generate():
        ...
        try:
            ...
            return {
                ...
                "raw": text if 'text' in locals() else "unknown",
            }
    ...
    def generate_stream(
        ...
        Yields:
            Token strings as they are received.
        """
        if not self.api_key:
...
class GeminiClient:
    ...
    def _get_model(self, model_name: str):
        """Lazy import and create Gemini model."""
